...
if [ "$OSTYPE" == "linux-gnu" ]; then
	DOCKER_BUILDKIT=1 docker build --no-cache --force-rm -t spring-petclinic:latest .
# No, we're building on Windows, native CMD or Powershell terminal
elif [ "$OSTYPE" == "win32" ]; then
        docker build --no-cache --force-rm -t spring-petclinic:latest .
# Wait, we're on Windows but we're building using msys termina (eg. git bash)
elif [ "$OSTYPE" == "msys" ]; then 
	winpty docker build --no-cache --force-rm -t spring-petclinic:latest .
else
